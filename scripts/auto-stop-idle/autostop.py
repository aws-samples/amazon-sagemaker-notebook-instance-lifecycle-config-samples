...
if len(data) > 0:
        ...
        if notebook['kernel']['execution_state'] == 'idle':
            if not ignore_connections:
                if notebook['kernel']['connections'] == 0:
                    ...
                    idle = False
                    print('Notebook idle state set as ', idle, ' because no kernel has been detected.')
            else:
                if not is_idle(notebook['kernel']['last_activity']):
                    idle = False
                    print('Notebook idle state set as ',idle, ' since kernel connections are ignored.')
        else:
    ...
    if not is_idle(uptime.strftime("%Y-%m-%dT%H:%M:%S.%fz")):
        idle = False
        print('Notebook idle state set as ',idle,' since no sessions detected.')
