...
try:
    ...
    if len(opts) == 0:
        raise getopt.GetoptError("No input parameter!")
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            logger.critical(helpInfo)
            exit(0)
        if opt in ("-t", "--time"):
            time = int(arg)
        if opt in ("-p", "--port"):
    ...
    logger.critical(usageInfo)
    exit(1)

# Missing configuration notification
missingConfiguration = False
if not time:
    logger.critical("Missing '-t' or '--time'")
...
def is_idle(last_activity):
    ...
    if (datetime.now() - last_activity).total_seconds() > time:
        logger.critical(f"Notebook is idle. Last activity time = {last_activity}")
        return True
    else:
...
if idle:
    logger.critical("Closing idle notebook")
    client = boto3.client("sagemaker")
    client.stop_notebook_instance(NotebookInstanceName=get_notebook_name())
else:
    logger.critical("Notebook not idle. Pass.")
